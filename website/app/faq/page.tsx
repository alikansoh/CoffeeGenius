 ...
 * app/faq/page.tsx
 *
 * FAQ page with SEO: generateMetadata + FAQPage JSON-LD + BreadcrumbList + WebPage JSON-LD.
 * Fixed: removed use of react-dom/server to avoid build error under Next.js/Turbopack.
 *
...
/**
 * Each FAQ entry provides:
 * - q: question (string)
 * - aHtml: an HTML string used for JSON-LD and for a11y-friendly plain-text extraction
 * - aNode: a ReactNode used for rendering in the UI (can include <Link />)
 *
 * We keep both to avoid using react-dom/server.renderToStaticMarkup while still rendering
 * rich internal links via Next's <Link /> in the client.
 */
const FAQS: Array<{ q: string; aHtml: string; aNode: React.ReactNode }> = [
  {
    q: "Do you ship internationally?",
    aHtml:
      `We ship within the UK. For international shipping, please contact us with your country and postal code and we'll confirm availability and costs. For quick questions email ${COMPANY_EMAIL}.`,
    aNode: (
      <>
        We ship within the UK. For international shipping, please contact us with your country and postal code and we’ll confirm
        availability and costs. For quick questions email{" "}
  ...
  },
  {
    ...
    aHtml:
      `Yes — we offer bespoke private and corporate classes. Tell us the number of guests, preferred dates, and any skill level or focus (espresso, latte art, filter brewing) and we'll prepare a proposal. Contact us at ${COMPANY_EMAIL}.`,
    aNode: (
      <>
  ...
  },
  {
    ...
    aHtml:
      `We supply restaurants, shops and offices with roasted coffee and barista training. For wholesale pricing and minimum order information, please visit our Wholesale page and complete the wholesale form — we will contact you directly.`,
    aNode: (
      <>
        ...
        <Link href="/wholesale" className="text-indigo-600 hover:underline">
          Wholesale page
        </Link>{" "}
        and complete the wholesale form — we will contact you directly.
      </>
    ),
  },
  {
    ...
    aHtml:
      "Perishable items (roasted coffee, fresh food) cannot usually be returned, but if there is a problem with your order contact us within 48 hours and we will do our best to make it right. Non-perishable items follow our returns procedure — please include your order number and reason for return when contacting us.",
    aNode: (
      <>
  ...
  },
  {
    ...
    aHtml:
      "We work directly with producers and trusted importers focused on traceability, quality and fair treatment. We pay premiums for high-quality lots and prioritise sustainable practices in our sourcing.",
    aNode: (
      <>
  ...
  },
  {
    ...
    aHtml:
      "Yes — we use Stripe to process payments securely. Card details are handled directly by Stripe's PCI-compliant infrastructure, and we never store full card numbers on our servers. All payment interactions occur over HTTPS and use Stripe's recommended integrations (Stripe Checkout or Elements) to keep sensitive data out of our systems. For more information about Stripe security, see Stripe documentation on security. If you have concerns about a payment or need a receipt, email COMPANY_EMAIL.",
    aNode: (
      <>
        ...
        . If you have concerns about a payment or need a receipt, email{" "}
        <a href={`mailto:${COMPANY_EMAIL}`} className="text-indigo-600 hover:underline">
          {COMPANY_EMAIL}
        </a>
        .
  ...
  },
  {
    ...
    aHtml:
      `To request access, correction, deletion or portability of your personal data, contact us at ${COMPANY_EMAIL}. We may ask you to verify your identity before fulfilling requests.`,
    aNode: (
      <>
...
/** simple helper to strip HTML tags and collapse whitespace */
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const description = `Frequently asked questions about ${COMPANY_NAME}: shipping, opening hours, classes, wholesale and payments.`;
...
export default function FAQPage() {
  ...
  const webPageJsonLd: Record<string, unknown> = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    name: `FAQ — ${COMPANY_NAME}`,
    description: `Frequently asked questions about ${COMPANY_NAME}.`,
    url: pageUrl,
    image: [ogUrl],
  };
          ...
          <section className="space-y-3">
            {FAQS.map((f, i) => (
              <details key={i} className="bg-white border border-gray-100 rounded-lg p-4" aria-labelledby={`faq-${i}`}>
                ...
                <div className="mt-3 text-gray-700">{f.aNode}</div>
              </details>
            ))}
          </section>
