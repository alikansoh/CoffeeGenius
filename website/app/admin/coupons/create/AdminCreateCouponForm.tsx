...
export default function AdminCreateCouponForm() {
  ...
  // Reset appliesTo when type has its own product picker
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setToast({
        message: err instanceof Error ? err.message : "Failed to create coupon",
        type: "error",
      });
      setSaving(false);
    }
            ...
            {selectedIds.map((id) => {
              ...
              return (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => toggleProductId(id, fieldPath)}
        ...
        <div className="border-2 border-gray-200 rounded-xl p-3">
          <div className="flex flex-col sm:flex-row gap-2 mb-3">
            ...
            <select
              value={productFilter}
              onChange={(e) =>
                setProductFilter(e.target.value as "all" | "coffee" | "equipment")
              }
              ...
              {filteredProducts.map((product) => {
                ...
                return (
                  ...
                  >
                    ...
                    {product.origin && (
                      <span className="text-[10px] text-gray-500">{product.origin}</span>
                    )}
                    <span
    ...
    <main className="min-h-screen bg-gray-50 pb-12">
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-4 sm:py-5">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Create Coupon</h1>
          <p className="text-xs sm:text-sm text-gray-600 mt-0.5">
            Set up a discount code customers enter at checkout, or an automatic promotion that applies itself.
            Every field below has a short explanation under it.
          </p>
        </div>
      </div>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      {errors["nthItem.nth"] && (
                        <p className="text-xs text-red-600 mt-1">{errors["nthItem.nth"]}</p>
                      )}
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-gray-900 mb-2">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      {errors["buyXGetY.x"] && (
                        <p className="text-xs text-red-600 mt-1">{errors["buyXGetY.x"]}</p>
                      )}
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-gray-900 mb-2">
                      ...
                      {errors["buyXGetY.y"] && (
                        <p className="text-xs text-red-600 mt-1">{errors["buyXGetY.y"]}</p>
                      )}
                    </div>
                  </div>
                  <p className="text-xs text-gray-600 italic">
            ...
            <section className="border-t-2 border-gray-100 pt-6">
              ...
              {showScopeProductPicker && (
                <ProductPicker
                  ...
                  label="Select products this coupon applies to"
                />
              )}
              {errors.productIds && (
                <p className="text-xs text-red-600 mt-1">{errors.productIds}</p>
              )}
            </section>
          </div>
