    ...
    <main className="min-h-screen bg-gray-50 py-6 px-4 sm:py-12 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <header className="mb-6">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Users — Admin</h1>
        ...
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
          <div className="flex-1 flex flex-col gap-2 sm:flex-row sm:items-center">
            <input
              ...
              className="w-full px-3 py-2 border rounded-md shadow-sm text-base"
            />
            <button
              onClick={() => {
                setSearch("");
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-2">
            ...
            <select
              ...
              className="px-2 py-2 border rounded-md text-base"
            >
            ...
            <button
              onClick={() => fetchUsers()}
              className="px-3 py-2 bg-sky-600 text-white rounded-md hover:bg-sky-700 transition-colors text-sm sm:text-base"
            >
              Refresh
            </button>
            <input
              ...
              className="px-3 py-2 border rounded-md text-base min-w-0 flex-1 sm:w-48"
            />
          </div>
        </div>

        {/* Form */}
        <form onSubmit={handleCreateOrUpdate} className="bg-white rounded-lg shadow p-4 sm:p-6 mb-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-4">
            ...
            {editingId && (
              <button
                type="button"
                onClick={resetForm}
                className="text-sm text-gray-500 hover:underline self-start sm:self-auto"
              >
                Cancel
              </button>
            )}
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="block w-full rounded-md border px-3 py-2 pr-10 text-base"
              ...
              <div className="mt-2 text-xs text-gray-600 flex items-center gap-2">
                <div className="w-24 h-2 bg-gray-200 rounded overflow-hidden">
                  <div
                    ...
                    className={`h-full transition-all duration-200 ${strength.score >= 3 ? "bg-green-500" : strength.score >= 2 ? "bg-yellow-400" : "bg-red-400"}`}
                  />
                </div>
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center">
            <button
              type="submit"
              disabled={saving}
              className="bg-sky-600 text-white px-4 py-2 rounded-md disabled:opacity-60 hover:bg-sky-700 transition-colors text-base font-medium"
            >
            ...
            {editingId && (
              <button
                type="button"
                onClick={() => resetForm()}
                className="px-4 py-2 rounded-md border hover:bg-gray-50 transition-colors text-base"
              >
                Cancel
              </button>
            )}
        ...
        <section className="bg-white rounded-lg shadow p-4 sm:p-6">
          ...
          {loading ? (
            ...
            <div className="text-sm text-gray-500 text-center py-8">No users found. Create one above.</div>
          ) : (
            <>
                  ...
                  <table className="w-full table-fixed border-collapse">
                    <thead>
                      <tr className="text-left text-sm text-gray-600 border-b">
                        <th className="py-2 px-2 w-1/2 min-w-32">Username</th>
                        <th className="py-2 px-2 w-1/3 min-w-24 hidden sm:table-cell">User ID</th>
                        <th className="py-2 px-2 w-1/6 min-w-20">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {visibleUsers.map((u) => (
                        <tr key={u._id} className="text-sm border-b last:border-b-0 hover:bg-gray-50">
                          ...
                          <td className="py-2 px-2 break-words text-xs text-gray-500 hidden sm:table-cell">{u._id}</td>
                          <td className="py-2 px-2">
                            <div className="flex gap-2 flex-col sm:flex-row">
                              <button
                                onClick={() => startEdit(u)}
                                className="px-3 py-1 text-sm bg-white border rounded hover:bg-gray-50 transition-colors min-h-8"
                              >
                                Edit
                              </button>
                              <button
                                onClick={() => setConfirmDelete({ id: u._id, username: u.username })}
                                className="px-3 py-1 text-sm bg-red-600 text-white rounded hover:opacity-90 transition-colors min-h-8"
                              >
                                Delete
                              </button>
                            </div>
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-2 self-center sm:self-auto">
                  <button
                    ...
                    disabled={page === 1}
                    className="px-3 py-2 border rounded disabled:opacity-50 hover:bg-gray-50 transition-colors min-h-10"
                  >
                    Prev
                  </button>
        ...
        {confirmDelete && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-lg shadow p-4 sm:p-6 w-full max-w-md mx-4">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
                <button onClick={() => setConfirmDelete(null)} className="px-4 py-2 rounded-md border hover:bg-gray-50 transition-colors order-2 sm:order-1">
                  Cancel
                </button>
                <button onClick={confirmDeleteUser} className="px-4 py-2 rounded-md bg-red-600 text-white hover:bg-red-700 transition-colors order-1 sm:order-2">
                  Delete
                </button>
              </div>
            </div>
        ...
        {/* Toasts */}
        <div className="fixed right-4 bottom-4 z-50 flex flex-col gap-2 max-w-sm w-full sm:w-auto">
          {toasts.map((t) => (
            <div
              key={t.id}
