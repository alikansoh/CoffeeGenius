...
export interface Equipment {
  ...
  category: string;
  price: number;
  imgUrl?: string;  // ✅ Full image URL from API
  images?: string[];
  totalStock?: number;
}
...
export default function AdminEquipmentList({ sendCookies = true }: AdminEquipmentListProps) {
  ...
  useEffect(() => {
    const fetchEquipments = async () => {
      setLoading(true);
      try {
        const res = await window.fetch("/api/equipment?limit=100", {
          method: "GET",
          headers: {
        ...
        setEquipments(data?.data || data?.equipments || data || []);
      } catch (err) {
        console.error(err);
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      const res = await window.fetch(`/api/equipment/${encodeURIComponent(id)}`, {
        method: "DELETE",
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-600 mt-0.5">
                    View, edit, and manage your equipment products
                  </p>
                </div>
              </div>
              <button
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b-2 border-gray-200">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-900 uppercase tracking-wide">Brand</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-900 uppercase tracking-wide">Category</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-900 uppercase tracking-wide">Price</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-900 uppercase tracking-wide">Stock</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {loading ? (
                    <tr>
                      <td colSpan={8} className="px-6 py-16 text-center">
                        <div className="flex flex-col items-center justify-center gap-3">
                          ...
                          <p className="text-sm text-gray-600">Loading equipments...</p>
                        </div>
                      </td>
                    </tr>
                  ) : filtered.length === 0 ? (
                      ...
                      <td colSpan={8} className="px-6 py-16 text-center">
                        <div className="flex flex-col items-center justify-center gap-3">
                          <p className="text-gray-900 font-medium">
                            {search ? "No equipments match your search" : "No equipments found"}
                          </p>
                          {!search && (
                            <button
                              onClick={() => router.push("/admin/equipment/create")}
                    ...
                    filtered.map((equipment) => (
                      <tr key={equipment._id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm text-gray-600">{equipment.brand}</div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm text-gray-600">£{equipment.price.toFixed(2)}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="text-sm text-gray-600">{equipment.totalStock ?? 0}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="text-sm text-gray-500 font-mono">{equipment.slug}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-end gap-2">
                            <button
                              onClick={() => router.push(`/admin/equipment/edit/${encodeURIComponent(equipment._id)}`)}
                ...
                >
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-gray-900 mb-1 truncate">{equipment.name}</h3>
                      <p className="text-sm text-gray-600 mb-1">{equipment.brand}</p>
                      <span className="inline-block px-2 py-1 bg-gray-100 text-gray-900 text-xs font-medium rounded-full capitalize mb-1">
                        {equipment.category}
                      </span>
                      <p className="text-sm text-gray-600 mb-1">£{equipment.price.toFixed(2)}</p>
                      <p className="text-xs text-gray-500 font-mono truncate">{equipment.slug}</p>
                    </div>
                  </div>
          ...
          {!loading && equipments.length > 0 && (
            <div className="mt-6 p-4 bg-white border-2 border-gray-200 rounded-xl">
              <div className="text-sm text-gray-600">
                ...
                <span className="font-bold text-gray-900">{equipments.length}</span> equipments
              </div>
            </div>
          )}
        </div>
