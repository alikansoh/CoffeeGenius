...
export default function AdminEditEquipmentPage() {
  ...
  // Fetch equipment record and detect video types BEFORE setting state
  const fetchEquipment = useCallback(async () => {
    ...
    try {
      ...
      const images: string[] = e.imagesPublicIds || (e.imgPublicId ? [e.imgPublicId] : []);

      // Detect video types BEFORE setting formData
      const detectedVideoMap = await detectAllPublicIds(images);

      const specsArray = Object.entries(e.specs || {}).map(([key, value]) => ({ key, value: value as string }));

      const formatted: EquipmentData = {
        _id: e._id || "",
        slug: e.slug || "",
        name: e.name || "",
        brand: e.brand || "",
        ...
        img: e.imgPublicId || "",
        images: images,
        totalStock: e.stock ? e.stock.toString() : "",
        notes: e.notes || "",
      ...
      setToast({ type: "error", message: "Failed to load equipment" });
    } finally {
      setLoading(false);
    }
  ...
  const setField = (k: keyof EquipmentData, v: EquipmentData[keyof EquipmentData]) =>
    setFormData((p) => ({ ...p, [k]: v }));
  ...
  const handleInputChange = (
    ...
    if (name === "price") {
      ...
      const n = value === "" || Number.isNaN(parsed) ? "" : Math.max(0, parsed).toString();
      setField("price", n);
      setErrors((s) => ({ ...s, price: "" }));
      return;
    }
    ...
    setField(name as keyof EquipmentData, value);
    setErrors((s) => ({ ...s, [name]: "" }));
  };
  ...
  const addFeature = () => {
    ...
    setFormData((p) => ({
      ...p,
      features: Array.from(new Set([...p.features, ...parts])).slice(0, 12),
    }));
    setFeatureInput("");
  };

  const removeFeature = (f: string) => setFormData((p) => ({ ...p, features: p.features.filter((x) => x !== f) }));
  ...
  const handlePasteURL = () => {
    const publicId = prompt("Paste Cloudinary public ID (e.g., equipment/espresso-main):");
    if (publicId) {
  ...
  const validateAll = () => {
    ...
    if (formData.price !== "" && (Number.isNaN(p) || p < 0)) next.price = "Price must be >= 0";
    const s = parseInt(formData.totalStock, 10);
    if (formData.totalStock !== "" && (Number.isNaN(s) || s < 0)) next.stock = "Stock must be >= 0";
    setErrors(next);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        totalStock: formData.totalStock === "" ? undefined : parseInt(formData.totalStock, 10),
    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600 font-medium">Loading equipment...</p>
        </div>
      </main>
    );
  }
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4 sm:py-5">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Edit Equipment</h1>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      placeholder="e.g., Breville, La Marzocco"
            ...
            <section className="bg-white rounded-2xl border-2 border-gray-200 p-4 sm:p-6 shadow-sm">
              ...
              <div className="flex gap-2 mb-3">
                <input
                  ...
                  placeholder="e.g., Automatic, Programmable, Stainless Steel"
